...
def obter_pedidos_melhor_envio():
    url = 'https://www.melhorenvio.com.br/api/v2/me/shipment'
    ...
    if response.status_code == 200:
        ...
        if novo_access_token:
            ...
            if response.status_code == 200:
                ...
                print(f"Erro ao obter pedidos após renovação: {response.status_code} - {response.text}")
        else:
            print("Não foi possível renovar o access token.")
    else:
...
def atualizar_planilha_google_sheets(pedidos, clientes, worksheet):
    ...
    for pedido in pedidos:
        ...
        # Obtendo dados de rastreamento
        rastreamento = obter_dados_rastreamento(id_pedido)
        codigo_rastreio = rastreamento.get('tracking', {}).get('code', 'N/A')
        ...
        if id_pedido not in ids_adicionados:
            lista_pedidos.append([
                ...
                codigo_rastreio  # Adicionando o código de rastreio à lista
            ])
